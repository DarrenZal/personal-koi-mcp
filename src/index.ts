...
// Vault operations for local Obsidian vault
import * as vault from './vault.js';
// Entity extraction and linking
import { buildExtractionPrompt, VaultEntity } from './entity-extraction.js';
import { createResolver } from './entity-resolver.js';
      ...
      try {
        ...
        switch (name) {
          ...
          case 'vault_search_notes':
            result = await this.vaultSearchNotes(args as { query: string; entityType?: string; searchContent?: boolean; limit?: number });
            break;
          case 'vault_get_entity':
            result = await this.vaultGetEntity(args as { entityType: string; name: string });
            break;
          case 'vault_prep_meeting':
            result = await this.vaultPrepMeeting(args as { attendees: string[]; project?: string; limit?: number });
            break;
          case 'vault_extract_entities':
            result = await this.vaultExtractEntities(args as { path?: string; content?: string; entityTypes?: string[] });
            break;
          case 'vault_process_extraction':
            result = await this.vaultProcessExtraction(args as { path: string; response: string; preview?: boolean; createEntities?: boolean; minConfidence?: number });
            break;
          case 'vault_find_backlinks':
            result = await this.vaultFindBacklinks(args as { name: string; fuzzy?: boolean; threshold?: number; limit?: number });
            break;
          case 'vault_query_frontmatter':
            result = await this.vaultQueryFrontmatter(args as { field: string; value: string; entityType?: string; fuzzy?: boolean; threshold?: number; limit?: number });
            break;
          case 'vault_find_person':
            result = await this.vaultFindPerson(args as { name: string; fuzzy?: boolean; threshold?: number; limit?: number });
            break;
...
Your feedback helps improve KOI for everyone.${
  ...
  /**
   * Read a note from the local Obsidian vault
   */
  private async vaultReadNote(args: { path: string }): Promise<{ content: Array<{ type: string; text: string }> }> {
    try {
      const result = await vault.readNote(args.path);
      ...
      const output = {
        ...
        entityType: result.parsed.entityType,
        entityId: result.parsed.entityId,
        frontmatter: result.parsed.frontmatter,
        wikilinks: result.parsed.wikilinks,
        body: result.parsed.body.slice(0, 2000) + (result.parsed.body.length > 2000 ? '...' : '')
      };
  ...
  /**
   * Write a note to the local Obsidian vault
   */
  private async vaultWriteNote(args: {
    path: string;
    content: string;
    ...
    try {
      ...
      if (result.success) {
        ...
        return {
          content: [{
            type: 'text',
            text: `Failed to write note: ${result.error}`
          }]
        };
      }
    } catch (error) {
      return {
        content: [{
          type: 'text',
          text: `Error writing note: ${error instanceof Error ? error.message : 'Unknown error'}`
        }]
      };
    }
  }
   ...
   */
  private async vaultListNotes(args: {
    folder?: string;
    entityType?: string;
    limit?: number;
  }): Promise<{ content: Array<{ type: string; text: string }> }> {
   ...
   */
  private async vaultSearchNotes(args: {
    query: string;
    entityType?: string;
    ...
    try {
      const notes = await vault.searchNotes({
        query: args.query,
        entityType: args.entityType,
        searchContent: args.searchContent ?? true,
        limit: args.limit || 20
      });
  ...
  /**
   * Get a note by entity type and name
   */
  private async vaultGetEntity(args: {
    entityType: string;
    name: string;
  }): Promise<{ content: Array<{ type: string; text: string }> }> {
    try {
      const result = await vault.getNoteByEntity(args.entityType, args.name);

      if (!result.found) {
        return {
          content: [{
            type: 'text',
            text: `Entity not found: ${args.entityType} "${args.name}"`
          }]
        };
      }
      ...
      const output = {
        ...
        frontmatter: result.parsed?.frontmatter,
        wikilinks: result.parsed?.wikilinks,
        body: result.parsed?.body?.slice(0, 2000) + ((result.parsed?.body?.length || 0) > 2000 ? '...' : '')
      };
      ...
      return {
        content: [{
          type: 'text',
          text: `Error getting entity: ${error instanceof Error ? error.message : 'Unknown error'}`
        }]
      };
    }
  }
   ...
   */
  private async vaultPrepMeeting(args: {
    attendees: string[];
    project?: string;
    limit?: number;
  }): Promise<{ content: Array<{ type: string; text: string }> }> {
    try {
      const meetingLimit = args.limit || 5;
      const results: any = {
        attendees: [],
        pastMeetings: []
      };
      ...
      for (const attendee of args.attendees) {
        const entity = await vault.getNoteByEntity('Person', attendee);
        if (entity.found) {
          results.attendees.push({
            name: attendee,
            found: true,
            path: entity.path,
            frontmatter: entity.parsed?.frontmatter,
            wikilinks: entity.parsed?.wikilinks
          });
        } else {
          results.attendees.push({
            name: attendee,
      ...
      const meetingNotes = await vault.listNotes({
        folder: 'Meetings',
        limit: 200
      });

      // Filter to meetings involving attendees or project
      for (const meeting of meetingNotes) {
        if (results.pastMeetings.length >= meetingLimit) break;

        // Check if meeting involves any attendee
        const meetingResult = await vault.readNote(meeting.path);
        if (!meetingResult.exists) continue;

        const fm = meetingResult.parsed.frontmatter || {};
        const meetingAttendees = fm.attendees || [];
        const meetingProject = fm.project || '';

        const involvesAttendee = args.attendees.some(a =>
          meetingAttendees.some((ma: string) =>
            ma && ma.toLowerCase().includes(a.toLowerCase())
          ) ||
          meetingResult.content.toLowerCase().includes(a.toLowerCase())
        );

        const involvesProject = args.project &&
          (meetingProject.toLowerCase().includes(args.project.toLowerCase()) ||
           meetingResult.content.toLowerCase().includes(args.project.toLowerCase()));

        if (involvesAttendee || involvesProject) {
          results.pastMeetings.push({
            path: meeting.path,
            name: meeting.name,
            date: fm.date,
            project: fm.project,
      ...
      // Format output
      let output = `## Meeting Prep\n\n`;
      output += `### Attendees\n\n`;

      for (const a of results.attendees) {
        if (a.found) {
          output += `**${a.name}** (Found: ${a.path})\n`;
          if (a.frontmatter) {
            if (a.frontmatter.jobTitle) output += `- Title: ${a.frontmatter.jobTitle}\n`;
            if (a.frontmatter.affiliation) output += `- Affiliation: ${a.frontmatter.affiliation}\n`;
            if (a.frontmatter.email) output += `- Email: ${a.frontmatter.email}\n`;
          }
          if (a.wikilinks && a.wikilinks.length > 0) {
            output += `- Related: ${a.wikilinks.slice(0, 5).join(', ')}\n`;
          }
          output += `\n`;
        } else {
          output += `**${a.name}** (No note found)\n\n`;
        }
      }

      output += `### Past Meetings (${results.pastMeetings.length})\n\n`;

      for (const m of results.pastMeetings) {
        output += `**${m.name}**\n`;
        output += `- Date: ${m.date || 'Unknown'}\n`;
        if (m.project) output += `- Project: ${m.project}\n`;
        if (m.actionItems && m.actionItems.length > 0) {
          output += `- Action Items: ${m.actionItems.filter((i: any) => i).join('; ')}\n`;
        }
        if (m.decisions && m.decisions.length > 0) {
          output += `- Decisions: ${m.decisions.filter((d: any) => d).join('; ')}\n`;
        }
        output += `\n`;
      }
      ...
      return {
        content: [{
          type: 'text',
          text: `Error preparing meeting: ${error instanceof Error ? error.message : 'Unknown error'}`
        }]
      };
    }
  }
  ...
  /**
   * Extract entities from a document for linking
   */
  private async vaultExtractEntities(args: {
    path?: string;
    content?: string;
    entityTypes?: string[];
  }): Promise<{ content: Array<{ type: string; text: string }> }> {
    try {
      ...
      // Build vault entity index
      const vaultEntities = await vault.buildEntityIndex();

      // Convert to VaultEntity format for prompt
      const entities: VaultEntity[] = vaultEntities.map(e => ({
        name: e.name,
        type: e.type,
        path: e.path,
        aliases: e.aliases
      ...
      const prompt = buildExtractionPrompt(documentContent, entities);

      // Return the prompt and stats for the LLM to process
      const output = {
        documentPath,
        documentLength: documentContent.length,
        vaultEntitiesLoaded: entities.length,
        entityTypes: args.entityTypes || ['Person', 'Organization', 'Location', 'Project', 'Concept'],
        prompt: prompt,
        instructions: 'Send this prompt to an LLM to extract entities. Then use vault_process_extraction with the response.'
      };
      ...
      return {
        content: [{
          type: 'text',
          text: `Error extracting entities: ${error instanceof Error ? error.message : 'Unknown error'}`
        }]
      };
    }
  }
  ...
  /**
   * Process an LLM extraction response and resolve entities
   */
  private async vaultProcessExtraction(args: {
    path: string;
    response: string;
    preview?: boolean;
    ...
    try {
      // Read the document
      const docResult = await vault.readNote(args.path);
      if (!docResult.exists) {
        return {
          content: [{
            type: 'text',
      ...
      const processor = createProcessor({
        ...
        createEntities: args.createEntities ?? false,
        minWikilinkConfidence: args.minConfidence ?? 0.7
      });

      processor.loadVaultEntities(entities);
      ...
      // Format summary
      const summary = processor.formatResultSummary(result);

      // If not preview and we have changes, apply them
      if (!args.preview && result.modifiedContent) {
        // Update the document with wikilinks
        await vault.writeNote(args.path, result.modifiedContent);
      }

      // If creating entities, create the new entity files
      if (args.createEntities && result.newEntities.length > 0) {
        for (const newEntity of result.newEntities) {
          await vault.writeNote(
            newEntity.path,
            newEntity.content,
            newEntity.frontmatter
          );
        }
      }
      ...
      return {
        content: [{
          type: 'text',
          text: `Error processing extraction: ${error instanceof Error ? error.message : 'Unknown error'}`
        }]
      };
    }
  }
   ...
   */
  private async vaultFindBacklinks(args: {
    name: string;
    fuzzy?: boolean;
    threshold?: number;
    limit?: number;
    ...
    try {
      const backlinks = await vault.findBacklinks(args.name, {
   ...
   */
  private async vaultQueryFrontmatter(args: {
    field: string;
    value: string;
    entityType?: string;
    fuzzy?: boolean;
    ...
    try {
      const notes = await vault.queryByFrontmatter({
        field: args.field,
        value: args.value,
        entityType: args.entityType,
        fuzzy: args.fuzzy ?? true,
      ...
      return {
        content: [{
          type: 'text',
          text: `Error querying frontmatter: ${error instanceof Error ? error.message : 'Unknown error'}`
        }]
      };
    }
  }
   ...
   */
  private async vaultFindPerson(args: {
    name: string;
    fuzzy?: boolean;
    threshold?: number;
    limit?: number;
    ...
    try {
      ...
      // 1. Find person note (with fuzzy matching)
      const personEntity = await vault.getNoteByEntity('Person', args.name);
      if (personEntity.found) {
        results.personNote = {
          path: personEntity.path,
          frontmatter: personEntity.parsed?.frontmatter,
          wikilinks: personEntity.parsed?.wikilinks
        };
      } else if (fuzzy) {
        // Try fuzzy search in People folder
        const peopleNotes = await vault.listNotes({ folder: 'People', limit: 100 });
        const normalizedSearch = args.name.toLowerCase().replace(/\s+/g, ' ').trim();

        for (const note of peopleNotes) {
          const normalizedName = note.name.toLowerCase().replace(/\s+/g, ' ').trim();
          // Simple word overlap check
          const searchWords = new Set(normalizedSearch.split(' '));
          const noteWords = normalizedName.split(' ');
          const matchingWords = noteWords.filter(w => searchWords.has(w)).length;
          const matchRatio = matchingWords / Math.max(searchWords.size, noteWords.length);

          if (matchRatio >= threshold) {
            const noteContent = await vault.readNote(note.path);
            results.personNote = {
              path: note.path,
              name: note.name,
              matchConfidence: matchRatio,
              frontmatter: noteContent.parsed.frontmatter,
              wikilinks: noteContent.parsed.wikilinks
            };
            break;
          }
        }
      ...
      // 2. Find meetings where they're an attendee
      const meetingResults = await vault.queryByFrontmatter({
        field: 'attendees',
        value: args.name,
        fuzzy,
        threshold,
      ...
      results.meetings = meetingResults.map(m => ({
        path: m.path,
        name: m.name,
        date: m.frontmatter?.date,
        project: m.frontmatter?.project,
        actionItems: m.frontmatter?.actionItems,
        decisions: m.frontmatter?.decisions
      }));

      // 3. Find backlinks
      const backlinks = await vault.findBacklinks(args.name, { fuzzy, threshold, limit });
      results.backlinks = backlinks.map(b => ({
        path: b.path,
        name: b.name,
        linkText: b.linkText
      }));

      // 4. Search for content mentions
      const contentMentions = await vault.searchNotes({
        query: args.name,
        searchContent: true,
        limit
      });
      ...
      const existingPaths = new Set([
        results.personNote?.path,
        ...results.meetings.map((m: any) => m.path + '.md'),
        ...results.backlinks.map((b: any) => b.path + '.md')
      ].filter(Boolean));

      results.mentions = contentMentions
        .filter(n => !existingPaths.has(n.path))
        .map(n => ({
          path: n.path,
          name: n.name,
          entityType: n.entityType,
      ...
      let output = `# Person Search: ${args.name}\n\n`;

      if (results.personNote) {
        output += `## Person Note\n`;
        output += `**Path:** ${results.personNote.path}\n`;
        if (results.personNote.matchConfidence) {
          output += `**Match Confidence:** ${(results.personNote.matchConfidence * 100).toFixed(0)}%\n`;
        }
        if (results.personNote.frontmatter) {
          const fm = results.personNote.frontmatter;
          if (fm.jobTitle) output += `**Title:** ${fm.jobTitle}\n`;
          if (fm.affiliation) output += `**Affiliation:** ${fm.affiliation}\n`;
          if (fm.email) output += `**Email:** ${fm.email}\n`;
        }
        output += '\n';
      } else {
        output += `## Person Note\n_No person note found_\n\n`;
      }

      output += `## Meetings as Attendee (${results.meetings.length})\n`;
      for (const m of results.meetings.slice(0, 10)) {
        output += `- **${m.name}** (${m.date || 'no date'})`;
        if (m.project) output += ` [${m.project}]`;
        output += `\n`;
      }
      if (results.meetings.length > 10) {
        output += `_... and ${results.meetings.length - 10} more meetings_\n`;
      }
      output += '\n';

      output += `## Backlinks (${results.backlinks.length})\n`;
      for (const b of results.backlinks.slice(0, 10)) {
        output += `- [[${b.path}]] (${b.linkText})\n`;
      }
      if (results.backlinks.length > 10) {
        output += `_... and ${results.backlinks.length - 10} more backlinks_\n`;
      }
      output += '\n';

      output += `## Other Mentions (${results.mentions.length})\n`;
      for (const m of results.mentions.slice(0, 10)) {
        output += `- [[${m.path}]]`;
        if (m.entityType) output += ` (${m.entityType})`;
        output += `\n`;
      }
      if (results.mentions.length > 10) {
        output += `_... and ${results.mentions.length - 10} more mentions_\n`;
      }
      ...
      return {
        content: [{
          type: 'text',
          text: `Error finding person: ${error instanceof Error ? error.message : 'Unknown error'}`
        }]
      };
    }
  }
  ...
  /**
   * Ingest extracted entities into the personal KOI backend
   *
   * This tool sends Claude-extracted entities to the backend for:
   ...
   */
  private async vaultIngestExtraction(args: {
    path: string;
    entities: Array<{
      name: string;
      type: string;
    ...
    try {
      ...
      if (backendAvailable) {
        ...
        output += `**Document RID:** ${documentRid}\n`;
        output += `**Receipt:** ${response.receipt_rid}\n\n`;
        ...
        const newEntities = response.canonical_entities.filter(e => e.is_new);
        const resolvedEntities = response.canonical_entities.filter(e => !e.is_new);
      ...
      } else if (fallbackToVault) {
        // Backend unavailable - fall back to vault-only resolution
        logger.warn('Personal KOI backend unavailable, falling back to vault-only resolution');

        // Build vault entity index
        const vaultEntities = await vault.buildEntityIndex();
        const resolver = createResolver();
        resolver.loadEntities(vaultEntities.map(e => ({
          name: e.name,
          type: e.type,
          path: e.path,
          aliases: e.aliases
        ...
        // Resolve each entity against vault
        let output = `# Entity Resolution Results (Vault-Only Fallback)\n\n`;
        output += `**Document:** ${args.path}\n`;
        output += `**Note:** Personal KOI backend unavailable at http://localhost:8351\n`;
        output += `**Mode:** Vault-only resolution (no deduplication or storage)\n\n`;
        ...
        for (const entity of args.entities) {
          const result = resolver.resolve(entity.name, entity.type);

          if (result.matchType === 'new') {
            newCount++;
            const suggestedPath = resolver.getSuggestedPath(entity.name, entity.type);
            output += `- **${entity.name}** (${entity.type}) → 🆕 NEW → [[${suggestedPath}]]\n`;
          } else {
            resolvedCount++;
            const status = result.matchType === 'exact' ? '✅ EXACT' :
                          result.matchType === 'alias' ? '✅ ALIAS' :
                          `⚡ FUZZY (${(result.confidence * 100).toFixed(0)}%)`;
            output += `- **${entity.name}** (${entity.type}) → ${status} → [[${result.matchedTo}]]\n`;
          }
        }

        output += `\n## Statistics\n`;
        output += `- Entities processed: ${args.entities.length}\n`;
        output += `- Resolved to existing: ${resolvedCount}\n`;
        output += `- New entities suggested: ${newCount}\n`;
        output += `\n_Note: Entities were NOT stored in the knowledge base (backend unavailable)_\n`;
        ...
        // Backend unavailable and fallback disabled
        return {
          content: [{
            type: 'text',
            text: `Error: Personal KOI backend unavailable at http://localhost:8351 and fallback disabled.\n\nTo start the backend, run:\n\`\`\`\nlaunchctl load ~/Library/LaunchAgents/com.personal.koi-processor.plist\n\`\`\``
          }]
        };
      }
